...
NAS is a Network-attached-storage system that is connected to our lab computer (similar to an external hardrive) however it acts as its own computer. It is easy to extend the memory on it by simply adding new hard drives but thinks of them all as one large hard drive. We will use it to store backup data for all our projects and for lab member to store their data before leaving the lab. 
...
On a MAC:
1. First connect to NU VPN (if you are off campus)
...
6. Within "Network" you should see our synology IP "129.10.11.41"
7. Navigate within 
...
Within the "Projects" folder, you should create a folder for each project using the following syntax: **YYYY_<ProjectName>_<YourInitials>** (e.g., 2024_BSB_RangeWide_RG).  
